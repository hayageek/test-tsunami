...
@PluginInfo(type = PluginType.VULN_DETECTION, name = "Exposed Flyte Console Detector", version = "0.1", description = "This detector checks for occurrences of exposed flyte console server installations.", author = "am0o0", bootstrapModule = ExposedFlyteConsoleDetectorModule.class)
@ForWebService
public final class ExposedFlyteConsoleDetector implements VulnDetector {
              ...
              if (isVulnerable(networkService)) {
                detectionReport.addDetectionReports(
                    buildDetectionReport(
                        targetInfo,
                        networkService,
                        "Flyte Console is misconfigured and can be accessed publicly,"
  ...
  private boolean isVulnerable(NetworkService networkService) {
    ...
    String rootUrl = NetworkServiceUtils.buildWebApplicationRootUrl(networkService);
    try {
      FlyteProtoClient client = new FlyteProtoClient(rootUrl);
      //Run the RCE and check the status in loop, until MAX_TIMEOUT_FOR_RCE_IN_SECS
      client.runShellScript(payload.getPayload(),MAX_TIMEOUT_FOR_RCE_IN_SECS);
            ...
            Vulnerability.newBuilder()
                .setMainId(
                    VulnerabilityId.newBuilder()
                        ...
                        .setValue("FLYTE_CONSOLE_EXPOSED"))
                .setSeverity(severity)
                .setTitle("Exposed Flyte Console Server")
                .setDescription(description)
                .setRecommendation("Please disable public access to your flyte console instance."))
        .build();
  }
}
